...
import { Box, Button, Card, CardContent, CardHeader, CardMedia, Grid, Paper, Stack, Typography } from '@mui/material'
    ...
    {renarratedBlocks && renarratedBlocks.map((block) => (
      <Card key={block.id} variant='elevation' elevation={0} >
        ...
        <CardMedia>
          <Stack direction="row" spacing={1} justifyContent="center">
            {extractMedia(block.content).map((src, index) => (
              <img
                key={index}
                style={{ width: '50%', height: 'auto', objectFit: 'cover', padding: 0.5 }}
                src={src}
        ...
        <CardContent>
          ...
          <Paper variant="outlined" sx={{ p: 2, my: 3, bgcolor: '#1c1c1c', color: 'white' }}>
            {block.img && (
              <img
                src={block.img}
